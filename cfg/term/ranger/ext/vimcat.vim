...
" (c) Matthew Wozniski (mjw@drexel.edu)

if &cp | set nocp | endif

let s:type = 'cterm'
if &t_Co == 0 | let s:type = 'term' | endif

" Converts info for a highlight group to a string of ANSI color escapes {{{1
function! s:GroupToAnsi(groupnum)
  if ! exists("s:ansicache") | let s:ansicache = {} | endif
  ...
  if rv == "" || rv == -1 | let rv = 0 | endif
  if bd == "" || bd == -1 | let bd = 0 | endif
  if rv | let temp = bg | let bg = fg | let fg = temp | endif
  if fg == "" || fg == -1 | unlet fg | endif
  if bg == "" || bg == -1 | unlet bg | endif
  ...
  if bd | let retv .= ";1" | endif

      if exists('fg') && fg < 8  | let retv .= ";3" . fg
  elseif exists('fg') && fg < 16 | let retv .= ";9" . (fg - 8)
  elseif exists('fg')            | let retv .= ";38;5;" . fg
  else                           | let retv .= ";39" | endif

      if exists('bg') && bg < 8  | let retv .= ";4" . bg
  elseif exists('bg') && bg < 16 | let retv .= ";10" . (bg - 8)
  elseif exists('bg')            | let retv .= ";48;5;" . bg
  else                           | let retv .= ";49" | endif
